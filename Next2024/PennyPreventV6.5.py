...
sl.session_state['flag_analise'] = True  # variável que define se o chatbot está em uso ou não #! ´False´ quando chatbot não estiver em uso para não consumir creditos
...
def GraficoDados(dadosExibir):
    ...
    for dado in dadosExibir.columns:
        sl.markdown(f'''<h3 style="color: #0099BB">{dado}</h3>''', unsafe_allow_html=True)

        # Verifica se a coluna contém dados numéricos
        if dadosExibir[dado].dtype in ['int64', 'float64']:
            # Calcula a média dos dados numéricos
            media_dados = dadosExibir[dado].mean()
            sl.markdown(f"Média: {media_dados:.2f}")
            
            # Agrupa os dados por intervalos para facilitar a visualização
            dadosGraf = dadosExibir[dado].rolling(window=5).mean()  # Média móvel com janela de 5
            
            # Exibe o gráfico com os dados agrupados por média
...
def ExibirColunasAtuais(dadosAtuais, texto):
    ...
    sl.markdown(f'''<div style="color: #0099BB"> {cols} </div>''', unsafe_allow_html=True)
...
def EnviarAnalise(info, teste, predito):
    ...
    print('FLAG!!!!!!!!!!!!!!!!!!!!:',sl.session_state['flag_analise'])
...
with tab_sobre:
    ...
    sl.markdown('''
        <br><br>
        <center>
                <div style="font-weight: bold; font-size: 50px; color: #00DDDD">Projeto em conjunto com a empresa Reply:</div>
                <div style="background-color: #81cafa; color: white; width: 940px; height: 3px;"></div>
                <br>
        ...
        <center>
                <div style="font-weight: bold; font-size: 50px; color: #00DDDD">Funcionamento do sistema:</div>
                <div style="background-color: #82c9ff; color: white; width: 610px; height: 3px;"></div>
        </center>
                <br>
        ...
        <center>        
            <div style="font-weight: bold; font-size: 50px; color: #00DDDD">Objetivos do Projeto:</div>
            <div style="background-color: #82c9ff; color: white; width: 460px; height: 3px;"></div>
            <br>
